...
I_inj_1=ones([1 50]); % Specify the duration of the stimulus in terms of the no. of samples 
a=length(I_inj_1);
I_inj(1, 10:(9+a))=((5*10^(-6))*I_inj_1);

alpha_m(1)= 0.1*((25-V(1))/exp((25-V(1))/10)-1);
beta_m(1)=4.*exp(((-1)*V(1))/18);
alpha_n(1)=0.01*((10-V(1))/(exp((10-V(1))/10)-1));
beta_n(1)=0.0125*exp(-V(1)/80);
...
m(i) = m(i-1) + frac.*(alpha_m(i-1).*(1-m(i-1)) - (beta_m(i-1).*m(i-1)));
n(i) = n(i-1) + frac.*(alpha_n(i-1).*(1-n(i-1)) - (beta_n(i-1).*n(i-1)));
h(i) = h(i-1) + frac.*(alpha_h(i-1).*(1-h(i-1)) - (beta_h(i-1).*h(i-1)));

% Currents, calculate the current values

I_Na=(m(i).^3)*gNa.*h(i).*(V(i-1)-E_Na);
I_K=(n(i).^4)*gK.*(V(i-1)-E_K);
I_L=gL.*(V(i-1)-E_L);
I_ion(i)=I_inj(i)-(I_Na)-(I_K)-(I_L);

%Update the Voltage to the next value using Euler's method

V_new(i)=V(i-1)+(frac*(I_ion(i-1)./C_m));
end
